...
우선 페이지에서 변경사항이 일어났을 때 모든 클라이언트에게 반영하기 위해 사용가능한 기술들은 다음과 같다.
...
'즉시' 반영되어야 하는 것이기 때문에,
이 중 서버 → 클라이언트 방향의 실시간 Push 통신인 SSE가 가장 적합했다.
...
기존 `useState` + `useEffect` 패턴을 `useQuery`로 마이그레이션하여 SSE 캐시 무효화가 작동하도록 수정했습니다.
